    ...
    while (!IsNull (&FormSet->FormListHead, FormLink)) {
      ...
      while (!IsNull (&Form->StatementListHead, QuestionLink)) {
        ...
        if ((Question->Operand == EFI_IFR_ONE_OF_OP) 
          ...
          //
          ConstantFlag    = TRUE;
          if (!Skip && (Question->SuppressExpression != NULL)) {
            Status = EvaluateExpression (FormSet, Form, Question->SuppressExpression, &ConstantFlag);
            if (!EFI_ERROR (Status) && Question->SuppressExpression->Result.Value.b && ConstantFlag) {
              Skip = TRUE;
            }
          }
